    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          if (error && error.length > 512) error = error.slice(0, 512) + '...';
...
function loadPersonas(): Map<string, any> {
    ...
    if (fs.existsSync(personaPath)) {
      try {
        ...
        if (personaPath.endsWith('.yaml') || personaPath.endsWith('.yml')) {
          personaData = yaml.load(fileContent) as any;
        } else {
    ...
    switch (name) {
      ...
      case 'generate_image': {
        ...
        if (imageData) {
          ...
          const fallbackResponse = await makeOpenRouterRequest(
            [{ role: 'user', content: `Please generate this image: ${params.description}` }],
            params.model || IMAGE_MODEL,
            0.9,
            512
          );
      ...
      case 'writers_room': {
        ...
        if (mode === 'chat') {
          ...
          for (const [key, p] of entries) nameToKey[(p as any).name.toLowerCase()] = key;
          
          let targetKey: string;
          let targetPersona: any;
...
Respond in character, focusing on your area of expertise. Be constructive but honest.`;
          ...
          let personasToUse: string[];
          if (Array.isArray(params.personas) && params.personas.length > 0) {
            personasToUse = params.personas.map((p: string) => {
              const lowered = String(p).toLowerCase();
              if (personas.has(p)) return p; // allow raw key if provided
              if (nameToKey[lowered]) return nameToKey[lowered];
              throw new Error(`Unknown persona: ${p}. Available: ${entries.map(([, v]: any) => v.name).join(', ')}`);
            });
          } else {
